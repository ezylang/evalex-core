  ...
  public EvaluationValue evaluate(
    ...
    if (parameterValues[0].getNumberValue().signum() < 0) {
      throw new EvaluationException(
          functionToken,
          String.format("Parameter to %s must not be negative", functionToken.getValue()));
    }

    return new EvaluationValue(
        parameterValues[0].getNumberValue().sqrt(expression.getConfiguration().getMathContext()));
  }
}
