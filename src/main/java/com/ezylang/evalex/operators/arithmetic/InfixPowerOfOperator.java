  ...
  public EvaluationValue evaluate(
    ...
    if (leftOperand.isNumberValue() && rightOperand.isNumberValue()) {
      ...
      v2 =
          v2.multiply(
              new BigDecimal(
                  signOf2, expression.getConfiguration().getMathContext())); // n2 is now positive
      BigDecimal remainderOf2 = v2.remainder(BigDecimal.ONE);
      BigDecimal n2IntPart = v2.subtract(remainderOf2);
      BigDecimal intPow =
          v1.pow(n2IntPart.intValueExact(), expression.getConfiguration().getMathContext());
      BigDecimal doublePow = BigDecimal.valueOf(Math.pow(dn1, remainderOf2.doubleValue()));

      BigDecimal result =
          intPow.multiply(doublePow, expression.getConfiguration().getMathContext());
      if (signOf2 == -1) {
        result =
            BigDecimal.ONE.divide(
                result, MathContext.DECIMAL128.getPrecision(), RoundingMode.HALF_UP);
      }
