...
class ExpressionConfigurationTest {
  ...
  void testDefaultSetup() {
    ...
    assertThat(configuration.getDataAccessor()).isInstanceOf(MapBasedDataAccessor.class);
  ...
  @Test
  void testCustomDataAccessor() {
    DataAccessorIfc mockedDataAccessor = Mockito.mock(DataAccessorIfc.class);

    ExpressionConfiguration configuration =
        ExpressionConfiguration.builder().dataAccessor(mockedDataAccessor).build();

    assertThat(configuration.getDataAccessor()).isEqualTo(mockedDataAccessor);
  }
